...
class CbcProductsService(CbcTypeService):
    ...
    def upload_single(self, model: munch.Munch):
        try:
            ...
            for image in images:
                ...
                self._ps.delete_image(self._resource, '%s/%s' % (model.prestashopId, image_id))
        except PrestashopItemNotFoundException as e:
            logger.debug('Resource [%s] id [%s] not found in db' % (self._resource, product.id))
    ...
    def transform_from_dict(self, dict: dict) -> munch.Munch:
        ...
        if not base.get('width') or not base.get('height') or not base.get('depth') or not base.get('weight'):
            raise ProductUploadException('Some dimention is missing for %s' % (base.id))

        if not base.get('priceWithDiscount'):
            raise ProductUploadException('Price with discount missing for %s' % (base.id))
        ...
        if (len(categories) == 0):
            raise ProductUploadException('No categories available for %s' % (base.id))
        ...
        if (manufacturer is None):
            raise ProductUploadException('No manufacturer available available %s' % (base.id))
    ...
    def find_by_id_or_reference(self, model: munch.Munch) -> PrestashopSearchElement:
        if not(model.get('prestashopId' is None)):
            product = self._ps.get(self._resource, resource_id=model.prestashopId)
    ...
    def update_stock(self, model: munch.Munch, product_xml: ElementTree):
        stock_id = product_xml[0].find('associations').find('stock_availables')[0].find('id').text
        data = self.create_model()
        data.quantity = model.quantity
        data.idProduct = product_xml[0].find('id').text
        data.stockId = stock_id

        self._ps.edit('stock_availables', data.stockId, compile('./handlebars/%s.xml' % ('stock_availables'), data.toDict()), {'id': stock_id})
